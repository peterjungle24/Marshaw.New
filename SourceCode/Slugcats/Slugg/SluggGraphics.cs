...
namespace SourceCode.Slugcats
{
    internal class SluggGraphics
    {
        ...
        private static void Initialize(On.RainWorld.orig_OnModsInit orig, RainWorld self)
        {
            ...
            {
                log.LogError($"{f(FunHelpers.RGB(255, 48, 48))}Thrown from <SourceCode.Slugcats/SluggGraphics/Initialize();>\n{ex}");
            }
        }
        private static void InitiateSprites(On.PlayerGraphics.orig_InitiateSprites orig, PlayerGraphics self, RoomCamera.SpriteLeaser sLeaser, RoomCamera rCam)
        {
            ...
            {
                log.LogError($"{f(FunHelpers.RGB(255, 48, 48))}Thrown from <SourceCode.Slugcats/SluggGraphics/InitiateSprites();>\n{ex}");
            }
        }
        private static void DrawSprites(On.PlayerGraphics.orig_DrawSprites orig, PlayerGraphics self, RoomCamera.SpriteLeaser sLeaser, RoomCamera rCam, float timeStacker, Vector2 camPos)
        {
            try
            {
                ...
                {
                    log.LogOnce($"<DrawSprites> lenght: {f(FunHelpers.RGB(207, 255, 48))}{sLeaser.sprites.Length}");
                }
            }
            catch (Exception ex)
            {
                log.LogError($"{f(FunHelpers.RGB(255, 48, 48))}Thrown from <SourceCode.Slugcats/SluggGraphics/DrawSprites();>\n{ex}");
            }
        }
        private static void HowThisAutocompleteTrigger(Player player) { }
    }
}
